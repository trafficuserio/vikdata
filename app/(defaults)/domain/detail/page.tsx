...
const promptModelChatGPTOptions = [
    { value: 'gpt-4o-mini', label: 'gpt-4o-mini' },
    { value: 'gpt-4o', label: 'gpt-4o' },
];
const promptModelClaudeOptions = [
    { value: 'claude-3-5-sonnet-20241022', label: 'claude-3-5-sonnet-20241022' },
    { value: 'claude-3-5-sonnet-20240620', label: 'claude-3-5-sonnet-20240620' },
];
const promptModelPerplexityOptions = [
...
export default function DomainDetail() {
    const columns: DataTableColumn<ExcelRow>[] = [
        { accessor: 'id', title: 'ID', sortable: true, textAlignment: 'left', render: ({ id }) => <span>{id}</span> },
        { accessor: 'primary_key', title: 'Từ khoá chính', sortable: true, textAlignment: 'left' },
        {
            accessor: 'secondary_key',
            title: 'Từ khoá phụ',
            sortable: true,
            textAlignment: 'left',
            render: ({ secondary_key }) => {
                const formatted = secondary_key?.replace(/\n/g, ', ') || '';
                return (
                    <Tippy content={formatted}>
                        <span style={{ display: 'inline-block', maxWidth: '200px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{formatted}</span>
                    </Tippy>
                );
            },
        },
        { accessor: 'h1', title: 'H1', sortable: true, textAlignment: 'left' },
        { accessor: 'title', title: 'Title', sortable: true, textAlignment: 'left' },
        { accessor: 'description', title: 'Description', sortable: true, textAlignment: 'left' },
        { accessor: 'post_id', title: 'Post ID', sortable: true, textAlignment: 'left' },
        {
            accessor: 'url',
            title: 'URL',
            sortable: true,
        ...
        { accessor: 'link_in', title: 'Link IN', sortable: true, textAlignment: 'left' },
        { accessor: 'link_out', title: 'Link OUT', sortable: true, textAlignment: 'left' },
        { accessor: 'is_done', title: 'Trạng thái', sortable: true, textAlignment: 'left' },
        {
            accessor: 'action',
            title: 'Hành động',
            ...
            render: (record, index) => (
                <div className="flex items-center justify-center gap-2">
                    <button type="button" onClick={() => handleSingleDelete(index)}>
    ...
    const [sortStatus, setSortStatus] = useState<DataTableSortStatus>({ columnAccessor: 'primary_key', direction: 'asc' });
    ...
    const getModelOptions = (typeOption: { value: string; label: string } | null) => {
        if (!typeOption) return [];
        if (typeOption.value === 'chatgpt') return promptModelChatGPTOptions;
        if (typeOption.value === 'claude') return promptModelClaudeOptions;
        return promptModelPerplexityOptions;
    };
        ...
        const fetchData = async () => {
            try {
                ...
                if ([401, 403].includes(json.errorcode)) {
                    ...
                    logout();
                    return;
                } else if (json.errorcode === 200) setDomainInfo(json.data);
            } catch {}
        };
        if (domainId && token) fetchData();
    }, [domainId, token]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const apiUrl = `${domainInfo.domain}wp-json/custom-api/v1/get-excel-data/`;
        ...
        if (domainInfo?.domain) fetchData();
    }, [domainInfo?.domain]);

    const filteredAndSortedRecords = useMemo(() => {
        let filtered = data.filter((item) => (item.primary_key || '').toLowerCase().includes(search.toLowerCase()));
        if (sortStatus) {
    ...
    const generateCSVBlob = (): Blob | null => {
    ...
    const handleSyncData = async () => {
        if (!isSyncing) {
            ...
            if (!csvBlob) return;
            const file = new File([csvBlob], 'export.csv', { type: 'text/csv;charset=utf-8;' });
            const formData = new FormData();
            formData.append('file', file);
            formData.append('region', 'en-US');
            const typeSite = domainInfo?.group_site === 'Hình ảnh' ? 'hinhanh' : 'huongdan';
            formData.append('type_site', typeSite);
            const isCrawling = domainInfo?.group_site === 'Hình ảnh' ? 'True' : 'False';
            formData.append('is_crawling', isCrawling);
            formData.append('url', domainInfo?.domain);
            formData.append('max_workes', '4');
            formData.append('username', 'lichxanuoc');
            formData.append('password', 'nUZ4 uJRO dwQ8 AKN5 YLKv ZeLe');
            try {
                const response = await axios.post('http://192.168.1.133:5050/api/site', formData, {
            ...
            try {
                const response = await axios.post('http://192.168.1.133:5050/api/site/stop', null, {
                    httpAgent: new http.Agent({ keepAlive: true }),
                });
                setIsSyncing(false);
            } catch (error) {
    ...
    const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (ev: ProgressEvent<FileReader>) => {
            ...
            const structuredData = mapRawDataToObjects(rawData);
            if (structuredData.length > 0) {
                ...
                setPromptConfig({
                    ...
                    prompt_h1: {
                        content: r.prompt_h1 || '',
                        type: r.prompt_h1_type ? { value: r.prompt_h1_type, label: r.prompt_h1_type } : null,
                        model: r.prompt_h1_model ? { value: r.prompt_h1_model, label: r.prompt_h1_model } : null,
                    },
                    prompt_title: {
                        content: r.prompt_title || '',
                        type: r.prompt_title_type ? { value: r.prompt_title_type, label: r.prompt_title_type } : null,
                        model: r.prompt_title_model ? { value: r.prompt_title_model, label: r.prompt_title_model } : null,
                    },
                    prompt_meta: {
                        content: r.prompt_meta || '',
                        type: r.prompt_meta_type ? { value: r.prompt_meta_type, label: r.prompt_meta_type } : null,
                        model: r.prompt_meta_model ? { value: r.prompt_meta_model, label: r.prompt_meta_model } : null,
                    },
                    prompt_sapo: {
                        content: r.prompt_sapo || '',
                        type: r.prompt_sapo_type ? { value: r.prompt_sapo_type, label: r.prompt_sapo_type } : null,
                        model: r.prompt_sapo_model ? { value: r.prompt_sapo_model, label: r.prompt_sapo_model } : null,
                    },
                    prompt_captions: {
                        content: r.prompt_captions || '',
                        type: r.prompt_captions_type ? { value: r.prompt_captions_type, label: r.prompt_captions_type } : null,
                        model: r.prompt_captions_model ? { value: r.prompt_captions_model, label: r.prompt_captions_model } : null,
                    },
                    prompt_conclusion: {
                        content: r.prompt_conclusion || '',
                        type: r.prompt_conclusion_type ? { value: r.prompt_conclusion_type, label: r.prompt_conclusion_type } : null,
                        model: r.prompt_conclusion_model ? { value: r.prompt_conclusion_model, label: r.prompt_conclusion_model } : null,
                    },
                    prompt_internal: {
                        content: r.prompt_internal || '',
                        type: r.prompt_internal_type ? { value: r.prompt_internal_type, label: r.prompt_internal_type } : null,
                        model: r.prompt_internal_model ? { value: r.prompt_internal_model, label: r.prompt_internal_model } : null,
                    },
                    prompt_outline: {
                        content: r.prompt_outline || '',
                        type: r.prompt_outline_type ? { value: r.prompt_outline_type, label: r.prompt_outline_type } : null,
                        model: r.prompt_outline_model ? { value: r.prompt_outline_model, label: r.prompt_outline_model } : null,
                    },
                    prompt_trienkhai: {
                        content: r.prompt_trienkhai || '',
                        type: r.prompt_trienkhai_type ? { value: r.prompt_trienkhai_type, label: r.prompt_trienkhai_type } : null,
                        model: r.prompt_trienkhai_model ? { value: r.prompt_trienkhai_model, label: r.prompt_trienkhai_model } : null,
                    },
                });
    ...
    const handleSingleDelete = (key: number) => {
        setData((prev) => prev.filter((_, idx) => idx !== key));
    };

    const handleBulkDelete = () => {
        const keysToDelete = new Set(selectedRecords.map((item) => data.indexOf(item)));
        setData((prev) => prev.filter((_, idx) => !keysToDelete.has(idx)));
        setSelectedRecords([]);
    };
    ...
    return (
        <>
                    ...
                    <div className="invoice-table">
                        <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row md:items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <button type="button" className="btn btn-info gap-2 flex items-center" onClick={handleExport}>
                                    <IconPlus />
                                    <p className="hidden md:block">Export Excel</p>
                                </button>
                                <button type="button" className="btn btn-primary gap-2 flex items-center" onClick={() => setModalPromptConfig(true)}>
                                    <IconPlus />
                                ...
                                <button type="button" className="btn btn-danger gap-2 flex items-center" onClick={handleBulkDelete}>
                                    <IconTrash />
                            ...
                            <div className="flex flex-col gap-4 md:flex-row md:items-center">
                                ...
                                />
                                <button type="button" className="btn btn-success gap-2 flex items-center" onClick={handleSyncData}>
                                    <IconPlus />
                                    <p className="hidden md:block whitespace-nowrap">{isSyncing ? 'Tạm dừng' : 'Đồng bộ dữ liệu'}</p>
                                </button>
                            </div>
                        </div>
            ...
            <Transition appear show={modalPromptConfig}>
                <Dialog as="div" open={modalPromptConfig} onClose={() => setModalPromptConfig(false)}>
                    <Transition.Child enter="ease-out duration-300" enterFrom="opacity-0" enterTo="opacity-100" leave="ease-in duration-200" leaveFrom="opacity-100" leaveTo="opacity-0">
                        <div className="fixed inset-0" />
                    </Transition.Child>
                    <div className="fixed inset-0 bg-[black]/60 z-[999] overflow-y-auto">
                                ...
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-auto max-w-3xl text-black dark:text-white-dark w-full min-w-[70vw] max-h-[80vh]">
                                            ...
                                            <div className="dropdown">
                                                ...
                                                >
                                                    <ul className="!min-w-[170px]">
                                                        ...
                                                        <li>
                                                            <button type="button">Mẫu Prompt 2</button>
                                                        </li>
                                                        <li>
                                                            <button type="button">Mẫu Prompt 3</button>
                                                        </li>
                                                        <li>
                                                            <button type="button">Mẫu Prompt 4</button>
                                                        </li>
                                                    </ul>
                                                </Dropdown>
                                            </div>
                                    ...
                                    <div className="p-5 space-y-2 custom-select">
                                            ...
                                            <div key={field.key} className="mb-4">
                                                ...
                                                <div className="flex gap-2 items-start">
                                                    <textarea
                                                        ...
                                                        rows={6}
                                                        value={promptConfig[field.key]?.content || ''}
                                                        onChange={(e) => setPromptConfig((prev) => ({ ...prev, [field.key]: { ...prev[field.key], content: e.target.value } }))}
                                                    ></textarea>
                                                    {field.key !== 'prompt_system' && field.key !== 'prompt_keywords' && (
                                                        <div className="flex gap-2 flex-col">
                                                            <Select
                                                                options={promptTypeOptions}
                                                                value={promptConfig[field.key]?.type}
                                                                onChange={(option) => setPromptConfig((prev) => ({ ...prev, [field.key]: { ...prev[field.key], type: option, model: null } }))}
                                                                placeholder="Prompt Type"
                                                                className="whitespace-nowrap w-60"
                                                            />
                                                            <Select
                                                                options={getModelOptions(promptConfig[field.key]?.type)}
                                                                value={promptConfig[field.key]?.model}
                                                                onChange={(option) => setPromptConfig((prev) => ({ ...prev, [field.key]: { ...prev[field.key], model: option } }))}
                                                                placeholder="Prompt Model"
                                                                isDisabled={!promptConfig[field.key]?.type}
                                                                className="whitespace-nowrap w-60"
                                                            />
                                                        </div>
                                                    )}
                                        ...
                                        <div className="flex justify-end items-center mt-8">
                                            ...
                                            <button type="button" className="btn btn-primary ltr:ml-4 rtl:mr-4" onClick={() => setModalPromptConfig(false)}>
                                                Lưu
                                            </button>
                                        </div>
                                    </div>
