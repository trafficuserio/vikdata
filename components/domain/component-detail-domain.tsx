...
export default function DomainDetail() {
            ...
            axios
                ...
                .then((response) => {
                    ...
                    servers.forEach((server: any) => {
                        ...
                        const serverObj: ServerStatus = { id: server.id, url: serverUrl, loading: true, is_running: true, has_temp: null, timedOut: false, domain_id: server.domain_id };
                        setServerData((prev) => [...prev, serverObj]);
                        axios
                            .get(`http://${serverUrl}:5050/api/site/status`, {
                                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                            })
                            .then((res) => {
                                const status = res.data;
                                setServerData((prev) => prev.map((item) => (item.url === serverUrl ? { ...item, is_running: status.is_running, loading: false } : item)));
                                setIsServerRunning(status.is_running);
                            })
                            .catch(() => {
                                setServerData((prev) => prev.map((item) => (item.url === serverUrl ? { ...item, is_running: true, loading: false } : item)));
                            });
                        let didTimeout = false;
                        const timeoutId = setTimeout(() => {
                            didTimeout = true;
                            setServerData((prev) => prev.map((item) => (item.url === serverUrl ? { ...item, timedOut: true, loading: false, has_temp: undefined } : item)));
                        }, 3000);
                        axios
                            .get(`http://${serverUrl}:5050/api/site/check`, {
                                params: { url: domainInfo.domain },
                                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                            })
                            .then((res) => {
                                if (!didTimeout) {
                                    ...
                                    setServerData((prev) =>
                                        prev.map((item) =>
                                            item.url === serverUrl ? { ...item, is_delete: res.data.is_delete === true || res.data.is_delete === 'true', timedOut: false, loading: false } : item,
                                        ),
                                    );
                                }
                            })
                            .catch(() => {
                                if (!didTimeout) {
                                    clearTimeout(timeoutId);
                                    setServerData((prev) => prev.map((item) => (item.url === serverUrl ? { ...item, is_delete: false, timedOut: true, loading: false } : item)));
                                }
                            });
                    });
                })
    ...
    const handleSyncData = async () => {
        if (!isSyncing) {
            ...
            formData.append('is_crawling', 'True');
            ...
            if (availableServer) {
                try {
                    ...
                    const response = await axios.post(`http://${availableServer.url}:5050/api/site`, formData, {
            ...
            if (activeServer) {
                try {
                    const response = await axios.post(`http://${activeServer.url}:5050/api/site/stop`, null, {
                        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    const handleDeleteTempData = async (serverUrl: string) => {
        ...
        try {
            await axios.delete(`http://${serverUrl}:5050/api/site/delete`, {
                params: { url: domainInfo.domain },
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
            });
    ...
    const handleDelete = async (record: { post_id: string; primary_key: string; url: string }) => {
        ...
        try {
            const response = await axios.post(
                `http://${serverUrl}:5050/api/site/delete_posts`,
                {
                    post_data: [
                        {
    ...
    const handleDeleteSelected = async () => {
        ...
        try {
            ...
            const response = await axios.post(`http://${serverUrl}:5050/api/site/delete_posts`, payload, {
                headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
            });
            if (response.status === 200) {
                        ...
                        <div className="mb-4.5 flex flex-col gap-5 px-5 md:flex-row justify-between items-start">
                            <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <button
                                    ...
                                    onClick={() => handleDeleteTempData(activeServer ? activeServer.url : domainInfo?.domain)}
                            ...
                            <div className="flex flex-col gap-4 md:flex-row md:items-center">
                                ...
                                <button
                                    type="button"
                                    className={`btn gap-2 flex items-center ${isServerRunning ? 'btn-primary' : importedExcelData.length === 0 || !allServerDomainNull ? 'btn-disabled' : 'btn-success'}`}
                            ...
                            <DataTable
                                ...
                                columns={[
                                    ...
                                    { accessor: 'is_done', title: 'Trạng thái', sortable: true, textAlignment: 'left' },
                                    {
                                        ...
                                        render: ({ url, post_id, primary_key }) => (
                                            <div className="flex items-center justify-center gap-2">
                                                {post_id != null && (
                                                    <>
                                                        ...
                                                        <button onClick={() => handleDelete({ url, post_id, primary_key })}>
